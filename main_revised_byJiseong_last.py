...
def IAT(time):  # 사람 간 간격
    ...
    if 0 <= time <= 30000:  # 화재 발생 초기에 사람들이 인지를 하는 시간 1분
        ...
        return_time = np.random.normal(1250/(num_people/num_exits), 10, 1)[0]
...
########### Total evacuation time의 히스토그램을 출력함
'''
plt.figure()
data=[np.round(person[i].Evacuation_time/1000) for i in range(len(person))]
bin=list(range(0,300,10))
plt.hist(data,bins=bin)
plt.grid()
plt.title('Histogram of Total Evacuation Time')
plt.xlabel('Total Evacuation time')
